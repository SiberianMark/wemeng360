 ...
 */
  ...
  class Wechat {
     ...
     */
    private $debug;
    protected $appid='wx42f8609fc962cd8d';
    protected $appsecrity='316a1471989c53945ec5d760f18960a8';
    public $_access_token; //微信token,高级接口使用
	/**
     * @var string 用户确认授权后获取用户基本信息的URL
     */
    public $URL_OAUTH2_USER_INFO = "https://open.weixin.qq.com/connect/oauth2/authorize?appid="+$appid+"&redirect_uri=
http://1.wemeng360.applinzi.com&response_type=code&scope=snsapi_userinfo&state=3";
    /**
    ...
    public function __construct($token, $debug = FALSE) {
      ...
      //获取微信access_token
        if (!S('cl_access_token')) {
            $result = httpGet('https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' . self::$APPID . '&secret=' . self::$APPSECRET);
            $result = json_decode($result, true);
            S('cl_access_token', $result['access_token'], $result['expires_in']);
            $this->_access_token = $result['access_token'];
        } else {
            $this->_access_token = S('cl_access_token');
        }
    }
